...
const token = "eyJhbGciOiJIUzI1NiJ9.eyJyb2xlIjpbIlJPTEVfTUFOQUdFUiJdLCJzdWIiOiJtaW5oIiwiaWF0IjoxNzQ3NzE5MDgzLCJleHAiOjE3NDc3NjIyODN9.YtD73bGkHHuRuF8bLfEncqk_Pv46gEyNdgbpdtDM0zQ";
...
function createActionButtons(employeeId) {
  return `
    <button class="btn btn-outline-info btn-sm me-1 btn-edit" data-id="${employeeId}">
      <i class="bi bi-eye"></i>
    </button>
    <button class="btn btn-outline-danger btn-sm btn-delete" data-id="${employeeId}">
...
function showEmployees(data) {
  ...
  clearAndRenderTable(filteredData, item => {
    ...
    row.innerHTML = `
      ...
      <td class="cell">${createActionButtons(item.userId)}</td>
    `;
...
// ========================== CUSTOMER ==========================
async function renderCustomerManagementPage() {
  ...
  document.querySelector("#add-items").addEventListener("click", openEmployeeForm);
...
function showCustomers(data) {
  clearAndRenderTable(data, item => {
    ...
    row.innerHTML = `
      ...
      <td class="cell">${createActionButtons()}</td>
    `;
    return row;
  });
}
